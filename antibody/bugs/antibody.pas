...
procedure LoadRejectList;
  ...
  begin
  c:=0;
  if ExistFile(ABCfg.Progpath+Msg^[58]^.t) then
      begin
      new(Reject.FileName);
...
Procedure KillOpenWorkDir;
  begin
  if existfile(ABCfg.ProgPath+Msg^[17]^.t) then
    begin
    {$I-}
...
Procedure OpenFiles;
  ...
  if existfile(ABCfg.ProgPath+Msg^[18]^.t) then append(log) else rewrite(log);
...
Procedure ProcessFile(var filename:string);
    ...
    if not (UpRec.Count=0) then
       for a:=1 to UpRec.Count do
         ...
         if upcasestr(Copy(UpRec.files[a].Name,1,pos('.',
             uprec.files[a].name)-1))=Upcasestr(Name)
          then UpRec.files[a].Name:=Name+Msg^[39]^.t;
         end;
    write_UpFile;
    Close_UpFile;
 ...
 procedure PreProcess;
  ...
  procedure NoPassVirus;
   ...
   {$I+}
   if existfile(filename) then erase(f);
   PruneDir(fexpand(ABCfg.ProgPath+Msg^[17]^.t));
   rmdir(fexpand(ABCfg.ProgPath+Msg^[17]^.t));
   end;
...
Procedure LoadArchiveDef;
  ...
  if not existfile(ABCfg.progpath+msg^[36]^.t) then
   begin
...
begin
 ...
 writeln(bf,'files opened');

 CheckKey;

 writeln(bf,'key checked');

 LoadArchiveDef;

 writeln(bf,'archive def list done');
 ...
 mpath:=Cfg.ProgPath+Msg^[37]^.t;

 writeln(bf,'mpath = "',mpath);
 ...
 if REG or (upcasestr(User.Name)=Msg^[90]^.t) then
   ...
   writeln(bf,'ff ',mpath + msg^[27]^.t);
   writeln(bf,'de ',doserror);
   While DosError=0 do
    begin
    pfn:=mpath+dosrec.name;
    writeln(bf,'dr.n ',dosrec.name);
     if not((DosRec.name='.') or (DosRec.Name='..')) and Not Allreadydone(dosrec.name)
        then
          begin
          writeln(bf,'* proc ',pfn); ProcessFile(pfn); end;
    FindNext(DosRec);
    end;
   end;
