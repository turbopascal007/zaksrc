...
procedure ShowSubboards;
  ...
  writeln('Joined to Indicated Subboards');
  ...
  if Cf.Ansi then TextColor ( Cf.ColorChart [ Normal ] );
    ...
    if subisreadable(p,user.access) and not(p^.fname='MAIL')
...
Procedure Proc(s:string);
  ...
  if (pos('+',ps)=0) and (pos('/',ps)=0) then exit;
...
Function GetCommands:boolean;
  ...
  writeln('Enter /SubName to UnJoin, +SubName to Join; [Enter] to Quit');
  write('Enter Command: ');
