Uses ExitErr,SlDriv,SLfLow,SLfHigh,Crt,Etc;
...
Function IfMember (PathToFiles,Subname : String):boolean; far;
 ...
 if not (SubName='MAIL') then
  begin
  if not Setup_Info (SubName,SetupMsg,tempsetup) then
   halt(1);

  If ((User.Access.MsgLevel >= TempSetup.Access) and
      (User.Access.Attrib   >= TempSetup.Attrib) and
      TempSetup.Visible)
   ...
   YesMbr := Not  (
      MemberOf ( PathtoFiles, Subname , User.Name )
      =0
     );
   ...
   write( lTab (length(subname)+1,12) );
...
MainSubList ( 'MAIN.SUB' , SetupMSG , IfMember );
