Program TapeFile; { (C) copyright 1993 by Zak Smith, All Rights Reserved }
...
Author Notes --
  ...
  Zak.Smith@xanadu.mil.wi.us   (or about anything else at the address)
}
...
Procedure BarfExit;
  ...
  LocalOnly;
  writeln;
  if portcheck and not carrier then writeln(' Carrier Lost');
  writeln;
  localandremote;
  KillStatusLine;
  Halt(1);
...
Procedure Pause;
   ...
   write('-- more -- ');
      ...
      until keypressed;
   Readkey;
   if portcheck and not carrier then BarfExit;
...
function GrabFromTape:integer;
  ...
  textcolor(lightgray);
  ExecStr:='RESTORE '+OrigPath+DlFile+' '+GenPath+' /V='+Tostrb(volume)+'/P/-O';
  KillStatusLine;
  ...
  {$IFNDEF DEBUG}
  err:=do_Exec(TapeSpec,ExecStr,USE_FILE,$ffff,FALSE);
  {$ENDIF}
  ...
  if err=0 then
    ...
    Writeln(LogFile, DtTmStamp, ' ',DlFile,' NOT Restored from tape, Error : ',Err:3);
...
Procedure SendZmodem(var SomeThingSent:boolean);
  ...
  ExecStr:='port '+tostrb(cfg.comport)+' sz '+GenPath+DlFile;
    ...
    if doserr=0 then
      ...
      inc(user.downloads,dir.length*128 div 1024);
      inc(user.dlcount);
...
Function CheckLimits: boolean;
  ...
  if (data^.rsactive or cfg.rsactive) then
   ...
   if (user.access.ratio>0) and
      ((((Setup.Value div 10 * Dir.Length*128 div 1024)+User.Downloads) div
       tv)>User.Access.Ratio)
...
Procedure ReadConfig;
 ...
 procedure proc;
  ...
  if hdr[1]<>';' then
   ...
   if hdr='DSZSPEC' then DSZSpec:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='TAPEFILEPATH' then ProgPath:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='TEMPBUFPATH' then GenPath:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='SUBBOARD' then DirFileName:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='VOLUMES' then NumOfVolumes:=toint(ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l)))))
   else if hdr='CONFIG' then PathToConfig:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='BACKUPPATH' then OrigPath:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
   else if hdr='TAPESPEC' then TapeSpec:=ltrim(rtrim(copy(l,pos(' ',l)+1,length(l)-pos(' ',l))))
 ...
 dirfilename := upcasestr(dirfilename);
...
begin
   If paramcount=0 then
    begin
    writeln('TapeFile [Text Config File Path]');
    halt(1);
    end
   else
   ...
   textbackground(black);
   textcolor(cyan);
   writeln('Please Wait... Will Take a few seconds...');
   writeln;
   InitConfig;
   Open_Strings;
   ...
   writeln('tape drive.   The list of these files is the '+dirfilename+' directory');
   ...
   textcolor(white);
   write('TapeFile ');
   textcolor(green);write(Version);
   textcolor(lightgray);write(' by ');textcolor(white);write('Zak Smith');
   textcolor(lightgray);write('; ');textcolor(cyan);write('Freeware');
   textcolor(lightgray);write('; ');
   textcolor(cyan);
   writeln('No portions copyright Searchlight Software');
